...
def split_edge(edge, suffix, suffix_tree):
    ...
    # alloc new edge
    new_edge = Edge(new_node_idx, edge.dst_node_idx,
                    edge.first_char_idx + len(suffix), edge.last_char_idx)
...
def canonize_suffix(suffix, suffix_tree):
    if not suffix.is_explicit():
        edge = suffix_tree.edge_lookup[
            suffix.src_node_idx, suffix_tree.string[suffix.first_char_idx]]
...
class SuffixTree:
    ...
    def insert_edge(self, edge):
        self.edge_lookup[
            edge.src_node_idx, self.string[edge.first_char_idx]] = edge

    def remove_edge(self, edge):
        del self.edge_lookup[
            edge.src_node_idx, self.string[edge.first_char_idx]]
...
def add_prefix(last_char_idx, active_point, suffix_tree):
    ...
    while True:
        ...
        if active_point.is_explicit():
            ...
            edge = suffix_tree.edge_lookup[
                active_point.src_node_idx, suffix_tree.string[active_point.first_char_idx]]
        ...
        new_edge = Edge(
            parent_node_idx, len(suffix_tree.nodes) - 1, last_char_idx, POSITIVE_INFINITY)
        suffix_tree.insert_edge(new_edge)
        # add suffix link
        if last_parent_node_idx > 0:
            suffix_tree.nodes[
                last_parent_node_idx].suffix_link = parent_node_idx
        last_parent_node_idx = parent_node_idx
        if active_point.src_node_idx == 0:
            active_point.first_char_idx += 1
        else:
            active_point.src_node_idx = suffix_tree.nodes[
                active_point.src_node_idx].suffix_link
...
def is_valid_suffix_tree(suffix_tree):
    ...
    if leaf_sum != len(suffix_tree.string):
        ...
        return False
    # root dosn't have edge leading to it
    if branch_sum != len(suffix_tree.nodes) - 1:
    ...
    for c in suffix_tree.alphabet:
        try:
            ...
            if walk_tree(suffix_tree, edge.dst_node_idx, current_suffix, l):
                ...
                # leaves have '-1' children
                branch_count[edge.dst_node_idx] -= 1
        except KeyError:
            pass
    if edges == 0:
        # leaf. check suffix
        is_valid_suffix[current_suffix_len] = ''.join(
            current_suffix[i] for i in range(current_suffix_len)) == suffix_tree.string[-(current_suffix_len):]
